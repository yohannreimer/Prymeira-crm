...
import { FileText, RotateCcw, Save, Target, Workflow } from "lucide-react";
...
const SettingsFormFields = () => {
  ...
  const validateDealTypes = useCallback(
    (types: { value: string; label: string }[] | undefined) =>
      validateItemsInUse(types, deals, "deal_type", dealTypeDisplayName, {
        duplicate: (displayName, duplicates) =>
          translate("crm.settings.validation.duplicate", {
            display_name: displayName,
            items: duplicates.join(", "),
    ...
    [dealTypeDisplayName, deals, translate],
  );
        ...
        <Card id="automations">
          <CardContent className="space-y-3">
            <h2 className="text-xl font-semibold text-muted-foreground">
              {translate("crm.settings.automations", { _: "Automações" })}
            </h2>
            <p className="text-sm text-muted-foreground">
              Ajuste regras comerciais automáticas, prazos e tarefas criadas
              pelo CRM.
            </p>
            <div className="flex flex-wrap gap-2">
              <CanAccess resource="automation_rules" action="edit">
                <Button asChild variant="outline">
                  <Link to="/automation_rules">
                    <Workflow className="h-4 w-4" />
                    {translate("crm.settings.automations", {
                      _: "Automações",
                    })}
                  </Link>
                </Button>
              </CanAccess>
              <CanAccess resource="proposal_templates" action="edit">
                <Button asChild variant="outline">
                  <Link to="/proposal_templates">
                    <FileText className="h-4 w-4" />
                    {translate("resources.proposal_templates.name", {
                      smart_count: 2,
                    })}
                  </Link>
                </Button>
              </CanAccess>
              <CanAccess resource="sales_goals" action="edit">
                <Button asChild variant="outline">
                  <Link to="/sales_goals">
                    <Target className="h-4 w-4" />
                    {translate("resources.sales_goals.name", {
                      smart_count: 2,
                    })}
                  </Link>
                </Button>
            ...
            <div className="flex flex-wrap gap-2">
              {dealStages?.map(
                (stage: { value: string; label: string }, idx: number) => {
                  const stageValue = stage.value || toSlug(stage.label);
                  const isSelected = dealPipelineStatuses.includes(stageValue);
                  return (
                    <Button
                      key={idx}
                      type="button"
