...
export const generateTasks = (db: Db) => {
  return Array.from(Array(400).keys()).map<Task>((id) => {
    ...
    const lead = random.arrayElement(db.leads);
    const deal = random.arrayElement(db.deals);
    const context = random.arrayElement(["contact", "contact", "lead", "deal"]);
    if (context === "contact") {
      contact.nb_tasks = (contact.nb_tasks ?? 0) + 1;
    }
    return {
      id,
      contact_id: context === "contact" ? contact.id : null,
      lead_id: context === "lead" ? lead.id : null,
      deal_id: context === "deal" ? deal.id : null,
