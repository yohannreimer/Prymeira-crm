...
describe("TaskCreateSheet", () => {
  it("creates a task for a selected contact and updates last_seen", async () => {
    ...
    await screen
      .getByLabelText(/descrição|description/i)
    ...
    await typeOptions.getByText(/Ligação|Call/).click();

    const dueDateInput = screen.getByLabelText(/data de vencimento|due date/i);
    await dueDateInput.clear();
    await dueDateInput.fill("2026-03-06T12:30");

    await screen.getByRole("button", { name: /^salvar$|^save$/i }).click();

    await expect
      .element(screen.getByText(/Tarefa adicionada|Task added/))
      .toBeInTheDocument();

    await expect
      .element(screen.getByText(/Criar tarefa|Create Task/))
